from __future__ import annotations  # legacy backup copy
...
def _extract_extras(r: ScoreBundle) -> Dict[str, float]:
    """
...
def format_console_table(
    ...
    for idx, r in enumerate(ranked, start=1):
        ...
        line = (
            f"{idx:>4}  "
            f"{r.symbol:<10}  "
            f"{_fmt_num(r.confluence_score, 1):>5}  "
...
def build_markdown_report(
    ...
    for idx, r in enumerate(ranked, start=1):
        ...
        row = (
            f"| {idx} "
            f"| {r.symbol} "
            f"| {_fmt_num(r.confluence_score, 1)} "
